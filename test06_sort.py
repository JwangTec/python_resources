...
#直接插入排序:取出i + 1的元素 与前面排好序的元素从后向前比较，小于等于则将i+1元素插入到该位置（有一定序的时候效率最高）
...
#shell(希尔)排序，将其下标依次由长度/2，每次进行快速插入排序


#归并排序 空间换时间，多了一个tmp列表占用内存，但时间减少为n*logn
